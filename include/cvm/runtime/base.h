...
#define CVM_TYPE_SWITCH(typecode, typebits, DType, ...)                     \
  if (typecode == kDLInt) {                                                 \
    switch (typebits) {                                                     \
      case 8: {                                                             \
      ...
      default:                                                              \
        CHECK_FATAL() << "unknown size for int type: " << typebits          \
                      << ". only accept 8, 16, 32 or 64 bits int\n";        \
    }                                                                       \
  } else if (typecode == kDLUInt) {                                         \
    switch (typebits) {                                                     \
      case 8: {                                                             \
      ...
      default:                                                              \
        CHECK_FATAL() << "unknown size for uint type: " << typebits         \
                      << ". only accept 8, 16, 32 or 64 bits int\n";        \
    }                                                                       \
  } else if (typecode == kDLFloat) {                                        \
    switch (typebits) {                                                     \
      case 32: {                                                            \
        typedef float DType;                                                \
        { __VA_ARGS__ }                                                     \
      ...
      default:                                                              \
        CHECK_FATAL() << "unknown size for float type: " << typebits        \
