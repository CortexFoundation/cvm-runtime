...
class YAMRTQuantizer:
    def __init__(self, model, per_channel, reduce_range, mode, static, weight_qType, input_qType, tensors_range,
        ...
        is_weight_int8 = weight_qType == QuantType.QInt32
        ...
        self.input_qType = onnx_proto.TensorProto.INT32 if input_qType == QuantType.QInt32 else onnx_proto.TensorProto.UINT32
        self.weight_qType = onnx_proto.TensorProto.INT32 if weight_qType == QuantType.QInt32 else onnx_proto.TensorProto.UINT32
    ...
    def quantize_subgraph(self, subgraph, graph_key):
        ...
        sub_quanitzer = YAMRTQuantizer(warped_model,
                                      self.per_channel,
                                      self.reduce_range,
                                      self.mode,
                                      self.static,
    ...
    def quantize_model(self):
        ...
        if self.parent is None:
            _, initializers_not_found = YAMRTQuantizer.CleanGraphInitializers(self.model.graph(), self.model.model)
        ...
        for node in graph.node:
            ...
            if len(graph_attrs) > 0:
                ...
                for attr in node.attribute:
                    kv = {}
                    if attr.type == onnx.AttributeProto.GRAPH:
                        cleaned_sub_graph, sub_requesting_tensor_names = YAMRTQuantizer.CleanGraphInitializers(attr.g, model)
                        kv = {attr.name: cleaned_sub_graph}
                        requesting_tensor_names.update({gn: 1 for gn in sub_requesting_tensor_names})
                    elif attr.type == onnx.AttributeProto.GRAPHS:
                        cleaned_graphes = []
                        for subgraph in attr.graphs:
                            cleaned_sub_graph, sub_requesting_tensor_names = YAMRTQuantizer.CleanGraphInitializers(subgraph, model)
